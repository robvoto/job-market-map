...
LinkedIn uses HTTP and never Chromium. Production discovery is geography-first for ACT, NSW and QLD with a blank search term and exact native-ID dedupe. JMM owns exact 10-position LinkedIn source offsets and confirms apparent terminal pages instead of trusting one short/empty response. The default rolling window is 5 hours every 4 hours; ACT/NSW/QLD network pages may fetch concurrently, but all SQLite ingestion, JMM-008 primary-vacancy assignment, dedupe and cursor writes remain serialized through one coordinator. Discovery stores cards only; it does not open vacancy pages or fetch JDs. The direct public-HTML LinkedIn detail helper remains available behind JMM-003 for on-demand enrichment after cross-source matching decides which JD is actually needed. Runtime scheduling remains inside the existing JMM scheduler/collection lock; there is no second daemon or browser process.
