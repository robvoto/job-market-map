...
Normal ongoing SEEK collection runs **once daily** over the latest **1 day** and is card-only. Known SEEK source job IDs are batch-recognised before ingestion. If the current card carries no changed canonical market evidence, JMM only refreshes that SEEK posting's `last_seen_at`; it does not create another raw capture, rerun duplicate analysis or reopen the JD. If a known card has genuinely changed source-backed evidence such as salary, title, employer/location/work type or Quick Apply, that card is ingested once so the canonical market row stays current. Missing card evidence never clears a previously known value. Do not increase SEEK to 6- or 12-hour browser runs merely to chase freshness: the persistent SEEK browser can receive human/security challenges, while the daily pass already reconciles a full 24-hour source window. When the midnight SEEK slot coincides with a LinkedIn slot, the scheduler deliberately gives the fast LinkedIn HTTP refresh first priority and starts SEEK afterwards.
