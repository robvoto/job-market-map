...
Salary bounds are deliberately period- and currency-specific: `salary_min` and/or `salary_max` must be accompanied by an explicit `salary_period` (`hour`, `day`, `week`, `month`, or `year`) and three-letter `salary_currency` such as `AUD`. JMM matches only normalized salary rows with the same proven period and currency; it never converts or compares `$100/hour` with `$120,000/year`. Range matching uses overlap semantics: a candidate matches `salary_min` when its upper bound reaches that amount and matches `salary_max` when its lower bound is at or below that amount. Rows with unknown period/currency or unresolved normalization do not match.
