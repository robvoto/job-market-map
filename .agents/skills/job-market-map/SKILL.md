...
### Runtime and visible terminal
Agents may start, stop or restart the local JMM runtime when needed for the requested work. Preserve Rob's visible Windows PowerShell terminal and its live output: start local JMM through the existing `C:\Users\thewr\run_JMM.ps1` launcher, which runs the foreground `scripts/start-api.sh` path. Do not use detached `scripts/service.sh start`, `nohup`, or a background command to start the local runtime. When stopping JMM, stop its service processes without closing or killing the Windows terminal or its parent session. If the existing launcher cannot be used from the current environment, explain that specific limitation instead of silently starting JMM detached or opening a different terminal.
