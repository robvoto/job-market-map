...
Canonical `jobs` rows contain source vacancy facts and deterministic interpretations of source freshness evidence. Store an exact source date/time when available; otherwise convert a supported relative label using that card's `captured_at`. If neither exists, a source-query freshness boundary may be stored only when the query window is explicit and proven. Record the basis in `jobs.posted_at_basis` (`source_exact`, `source_relative`, or `search_window_bound`) and preserve the original label/window in `card_captures.raw_json`. A window boundary is a conservative lower bound, not an exact posting instant. Use `unknown` only when no usable evidence exists. Exact source timestamps remain mandatory for SEEK incremental crawl cutoffs; derived dates must never drive early pagination stops. Historical cleanup/backfill is an MVP maintenance task: by default, repair only jobs observed within the most recent 7 days. Do not spend time repairing older data unless Rob explicitly asks or older rows are required to protect current canonical integrity/tests. Collector lifecycle (`first_seen_at`, `last_seen_at`, `capture_count`, archive/compaction state) belongs in `job_observation_state`, not `jobs`. Keep the exact source work type in `employment_type`; do not infer Permanent/Contract basis from ambiguous labels or JD prose.
...
Agents must **not** start, stop, restart, relaunch, replace, kill, terminate, or take over the local JMM API/Admin service, scheduler, persistent browser service, or collection process unless Rob explicitly asks for that exact target and runtime action in the current conversation/task. Approval is action-specific and single-use: once the approved lifecycle action has been completed, it is consumed and must not be reused later. Do not infer permission from a coding, debugging, testing, deployment-preparation, Git, release, or health-check request. Do not create replacement launchers or wrapper scripts unless Rob explicitly asks for one.

The default agent behaviour is read-only runtime inspection: check service status, process/lock state, Admin/API status and durable logs; make code/config/test/doc changes as requested; then leave runtime lifecycle changes to Rob. If JMM is unexpectedly running or stopped, report that state and do not correct it. If a code change requires a restart to take effect, say so clearly and stop there unless Rob explicitly authorises that restart.
