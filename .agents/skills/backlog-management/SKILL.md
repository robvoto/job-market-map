...
- Intended shared backlog identity: `agent-backlog-access@robvoto-agent-platform-iam.gserviceaccount.com`. The current secure Sheets connector incorrectly reports/uses `cv-docs-editor@cv-system-drive-management.iam.gserviceaccount.com`; do not share the backlog with the CV/Docs identity or treat its 403 as the backlog user's result. Correct the connector configuration before reading or writing the backlog.
