...
def test_linkedin_partial_failure_does_not_immediately_retry_same_slot(monkeypatch):
    from collector import scheduler
    ...
    monkeypatch.setattr(
        scheduler,
        "get_cycle",
        lambda _source: {"cycle_key": cycle_key, "status": "PARTIAL_FAILURE"},
    )
    tz = ZoneInfo("Australia/Sydney")
    due, returned_key, _ = scheduler.SchedulerService.linkedin_due_context(
        datetime(2026, 9, 11, 22, 15, tzinfo=tz)
    )
    assert due is False
    assert returned_key == cycle_key
